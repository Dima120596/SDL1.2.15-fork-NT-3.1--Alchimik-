...
static int using_standard_timer = 0;
...
int SDL_SYS_TimerInit(void)
{
	...
	/*if ( ! timerID ) {
	 ...
	 /* ФИКС: Если timeSetEvent не сработал — используем SetTimer */
    if (!timerID) {
        /* Пробуем стандартный Windows-таймер */
        timerID = (UINT)SetTimer(NULL, 0, TIMER_RESOLUTION, (TIMERPROC)HandleAlarm);
        if (!timerID) {
            SDL_SetError("timeSetEvent() and SetTimer() both failed");
            return(-1);
        }
        /* Запоминаем, что используем стандартный таймер */
        using_standard_timer = 1;
    }
...
void SDL_SYS_TimerQuit(void)
{
	...
	if (timerID) {
        if (using_standard_timer) {
            KillTimer(NULL, timerID); //Стандартный таймер - Win32s.
        } else {
            timeKillEvent(timerID);
        }
    }
