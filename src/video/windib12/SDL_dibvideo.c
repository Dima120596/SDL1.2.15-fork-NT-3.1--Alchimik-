				 ...
				 int width, int height, int bpp, Uint32 flags)
 {
	 ...
	 */
	 ...
	 if ( ! (flags & SDL_OPENGL) ) {
		...
		if (!WinGRecommendDIBFormat(binfo)) {
			  // fallback
			   binfo->bmiHeader.biBitCount = 8;
			   binfo->bmiHeader.biPlanes = 1;
			   binfo->bmiHeader.biCompression = BI_RGB;
		}
		// Теперь binfo содержит рекомендуемые значения, включая biWidth, biHeight?
		// Нет, WinGRecommendDIBFormat не заполняет ширину/высоту. Их нужно установить после.
		binfo->bmiHeader.biWidth = video->w;
		binfo->bmiHeader.biHeight = -video->h; // top-down
		 ...
		 screen_bmp = WinGCreateBitmap(WinGCreateDC(), binfo, (void**)&video->pixels);
...
//#ifdef SDL_VIDEO_DRIVER_WING
    mdc = WinGCreateDC();                 // ← WinG
//#else
//    mdc = CreateCompatibleDC(hdc);           // ← оригинал
//#endif
    SelectObject(mdc, screen_bmp);
    for ( i=0; i<numrects; ++i ) {
//#ifdef SDL_VIDEO_DRIVER_WING
        WinGBitBlt(hdc, rects[i].x, rects[i].y, rects[i].w, rects[i].h,
...
static BOOL CheckPaletteEntry(LOGPALETTE *pal, int index, BYTE r, BYTE g, BYTE b)
{
 ...
 //#endif
	...
	// }
	 ...
	 mdc = WinGCreateDC();
	 SelectObject(mdc, screen_bmp);
     WinGSetDIBColorTable(mdc, firstcolor, ncolors, pal);
     

	  if ( moved_entries || !grab_palette ) {
		  //BitBlt(hdc, 0, 0, this->screen->w, this->screen->h,
		  //       mdc, 0, 0, SRCCOPY);
		  WinGBitBlt(hdc, 0, 0, this->screen->w, this->screen->h, 
			  mdc, 0, 0);
	  }
	 DeleteDC(mdc);
	 SDL_stack_free(pal);
...
static void DIB_WinPAINT(_THIS, HDC hdc)
{
	...
	mdc = WinGCreateDC();
	SelectObject(mdc, screen_bmp);
	WinGBitBlt(hdc, 0, 0, SDL_VideoSurface->w, SDL_VideoSurface->h,
							mdc, 0, 0);
