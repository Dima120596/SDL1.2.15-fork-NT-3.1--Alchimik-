...
//#define NO_GETDIBITS 1
#define NO_GAMMA_SUPPORT  1
  #if _WIN32_WCE < 420
				...
				int width, int height, int bpp, Uint32 flags)
{
	...
	*/
	...
	if ( screen_bmp != NULL ) {
		DeleteObject_Universal(screen_bmp);
	}
	if ( ! (flags & SDL_OPENGL) ) {
		...
		hdc = GetDC(SDL_Window);
		screen_bmp = CreateDIBSection_Universal(hdc, binfo, DIB_RGB_COLORS,
...
int DIB_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors)
{
	...
	SetDIBColorTable_Universal(mdc, firstcolor, ncolors, pal);
