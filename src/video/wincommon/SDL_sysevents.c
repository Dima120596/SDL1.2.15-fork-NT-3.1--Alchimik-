...
//#ifdef _WIN32
/* Замены для NT 3.1 */
#ifndef GetKeyboardLayout
WINAPI GetKeyboardLayout_NT31(DWORD idThread) {
    HKL hkl = 0;
    if (!hkl) {
        char layout[9];
        if (GetKeyboardLayoutName(layout)) {
            hkl = (HKL)layout; /* Для совместимости */
        } else {
            hkl = (HKL)0x04090409; /* US English */
        }
    }
    return hkl;
}
#define GetKeyboardLayout GetKeyboardLayout_NT31
#endif
#ifndef LoadImage
HANDLE WINAPI LoadImage_NT31(HINSTANCE hinst, LPCTSTR lpszName, UINT uType,
                                    int cxDesired, int cyDesired, UINT fuLoad) {
    if (uType == 1) {
        return (HANDLE)LoadIcon(hinst, lpszName);
    } else if (uType == 2) {
        return (HANDLE)LoadCursor(hinst, lpszName);
    }
    return NULL;
}
#define LoadImage LoadImage_NT31
#endif

#ifndef ToAsciiEx
/* ========== ЗАГЛУШКА ДЛЯ ToAsciiEx (Windows NT 3.1 compatible) ========== */
/* Тип функции ToAsciiEx */
typedef int (WINAPI *TOASCIIEX_PROC)(UINT, UINT, const BYTE*, LPWORD, UINT, HKL);
/* Глобальный указатель на функцию */
static TOASCIIEX_PROC pToAsciiEx = NULL;
/* Флаг, что мы уже пытались загрузить */
static int toasciiex_checked = 0;
/* Инициализация: пробуем загрузить ToAsciiEx из user32 */
static void InitToAsciiEx(void) {
    HMODULE hUser32; 
    if (toasciiex_checked) return;   
    hUser32 = LoadLibrary("USER32.DLL");
    if (hUser32) {
        /* Пробуем найти ToAsciiEx */
        pToAsciiEx = (TOASCIIEX_PROC)GetProcAddress(hUser32, "ToAsciiEx");       
        /* Не освобождаем библиотеку, т.к. она уже загружена в процесс */
        toasciiex_checked = 1;
    }
}
/* Наша замена для ToAsciiEx */
static int WINAPI ToAsciiEx_NT31(UINT uVirtKey, UINT uScanCode, const BYTE *lpKeyState,
                                 LPWORD lpChar, UINT uFlags, HKL dwhkl) {
    /* Если не инициализировали - делаем это сейчас */
    if (!toasciiex_checked) {
        InitToAsciiEx();
    }
    
    /* Если функция есть - используем её */
    if (pToAsciiEx) {
        return pToAsciiEx(uVirtKey, uScanCode, lpKeyState, lpChar, uFlags, dwhkl);
    }
    
    /* Иначе используем старую добрую ToAscii */
    return ToAscii(uVirtKey, uScanCode, lpKeyState, lpChar, uFlags);
}

/* Переопределяем ToAsciiEx на нашу функцию */
#define ToAsciiEx ToAsciiEx_NT31
#endif
...
static int GetCodePage(void)
{
    char   buff[8];
    int    lcid = MAKELCID(LOWORD(GetKeyboardLayout(0)), SORT_DEFAULT);
    int    cp = GetACP();  // резерв
    HMODULE hKernel = GetModuleHandle("kernel32.dll");
    /* Определяем систему — NT или 9x */
    if (GetVersion() & 0x80000000) {
        /* Windows 9x — используем ANSI */
		FARPROC pGetLocaleInfoA = GetProcAddress(hKernel, "GetLocaleInfoA");
        pGetLocaleInfoA(lcid, LOCALE_IDEFAULTANSICODEPAGE, buff, 8);
        cp = atoi(buff);
		
    } else
	{
		/* Windows NT — используем Unicode */
		wchar_t wbuff[8];
		FARPROC pGetLocaleInfoW = GetProcAddress(hKernel, "GetLocaleInfoW");
        pGetLocaleInfoW(lcid, LOCALE_IDEFAULTANSICODEPAGE, wbuff, 8);
        WideCharToMultiByte(CP_ACP, 0, wbuff, -1, buff, 8, NULL, NULL);
        cp = atoi(buff);
    }
    return cp;
} 
