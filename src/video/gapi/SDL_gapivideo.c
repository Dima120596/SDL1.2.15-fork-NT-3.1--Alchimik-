...
static void GAPI_WinPAINT(_THIS, HDC hdc)
{
    ...
    int bpp = 16; // we always use either 8 or 16 bpp internally
    HGDIOBJ prevObject;
    unsigned short *bitmapData; // Этот указатель больше не нужен для хранения данных
    HBITMAP hb;
    HDC srcDC;
    BITMAPINFO bmi = {0}; // Объявляем структуру на стеке

    // CreateDIBSection does not support 332 pixel format on wce
    if( gapi->gxProperties.cBPP == 8 ) return;

    // --- 1. Заполняем BITMAPINFO ---
    bmi.bmiHeader.biSize        = sizeof(BITMAPINFOHEADER);
    bmi.bmiHeader.biWidth       = gapi->w;
    // Важно: отрицательная высота означает top-down DIB (первая строка в буфере - верхняя)
    bmi.bmiHeader.biHeight      = -gapi->h; 
    bmi.bmiHeader.biPlanes      = 1;
    bmi.bmiHeader.biBitCount    = bpp;
    bmi.bmiHeader.biCompression = BI_RGB; // По умолчанию
    bmi.bmiHeader.biSizeImage   = (gapi->w * gapi->h * bpp) / 8;

    // Для 16 бит нужно указать маски цветов через битовые поля
    if( bpp == 16 )
    {
        bmi.bmiHeader.biCompression = BI_BITFIELDS;
        // Внимание: Маски в BITMAPINFO идут сразу после заголовка, как три DWORD.
        // Поэтому мы должны интерпретировать bmi.bmiColors как массив DWORD.
        // Размера BITMAPINFO (sizeof(BITMAPINFOHEADER) + 3*sizeof(RGBQUAD)) хватает.
        ((DWORD*)bmi.bmiColors)[0] = REDMASK;
        ((DWORD*)bmi.bmiColors)[1] = GREENMASK;
        ((DWORD*)bmi.bmiColors)[2] = BLUEMASK;
    }

    // --- 2. Создаём DDB (Device-Dependent Bitmap) через CreateDIBitmap ---
    // Эта функция создает битмап, совместимый с контекстом устройства hdc,
    // и инициализирует его данными из буфера gapi->buffer.
    hb = CreateDIBitmap(
        hdc,                    // DC, с которым будет совместим битмап
        &bmi.bmiHeader,         // Заголовок, описывающий формат
        CBM_INIT,               // Флаг: инициализировать битмап данными
        gapi->buffer,           // Указатель на исходные пиксельные данные (BGR/BGRA)
        &bmi,                   // Полная информация о формате
        DIB_RGB_COLORS          // Цвета заданы в формате RGB (или масками для BI_BITFIELDS)
    );

    if (!hb) {
        // Ошибка создания битмапа - ничего не выводим
        return;
    }

    // --- 3. Выводим битмап на экран ---
    srcDC = CreateCompatibleDC(hdc);
    if (srcDC) {
        prevObject = SelectObject(srcDC, hb);
        BitBlt(hdc, 0, 0, gapi->w, gapi->h, srcDC, 0, 0, SRCCOPY);
