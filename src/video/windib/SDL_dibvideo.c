...
#undef SDL_VIDEO_OPENGL             /* OpenGL (на всякий случай) */
...
/* Вспомогательная функция для переворота DIB. Вызывается ОДИН раз после создания поверхности! */
/* Преобразует Top-Down (первая строка = верх экрана) в Bottom-Up (первая строка = низ экрана) */
void ConvertToBottomUp(void* pixels, int width, int height, int bpp) {
    int row_size = width * (bpp / 8); // Размер строки в байтах
    int stride = (row_size + 3) & ~3; // Выравнивание до 4 байт (обязательно для DIB) 
	int i;
    char *buffer = (char*)pixels;
    char *temp_row = (char*)malloc(stride);
    if (!temp_row) return;

    for (i = 0; i < height / 2; i++) {
        char *upper = buffer + i * stride;
        char *lower = buffer + (height - 1 - i) * stride;
        memcpy(temp_row, upper, stride);
        memcpy(upper, lower, stride);
        memcpy(lower, temp_row, stride);
    }
    free(temp_row);
}
...
// #endif
	...
	// if ( ! (flags & SDL_OPENGL) ) {
		...
		// hdc = GetDC(SDL_Window);
		// screen_bmp = CreateDIBSection(hdc, binfo, DIB_RGB_COLORS,(void **)(&video->pixels), NULL, 0);		
		// ReleaseDC(SDL_Window, hdc);
		// SDL_free(binfo);
				...
				int width, int height, int bpp, Uint32 flags)
{
	...
	/* OpenGL special case (без изменений) */
	if ( SDL_Window &&
	     ((current->flags & ~SDL_ANYFORMAT) == (flags & ~SDL_ANYFORMAT)) &&
	     (current->format->BitsPerPixel == bpp) &&
	     (flags & SDL_OPENGL) && 
	...
	/* Recalculate bitmasks (без изменений) */
	if ( bpp == current->format->BitsPerPixel ) {
		video = current;
	} else {
		switch (bpp) {
		...
		video = SDL_CreateRGBSurface(SDL_SWSURFACE, 0, 0, bpp, Rmask, Gmask, Bmask, 0);
		if ( video == NULL ) {
			SDL_OutOfMemory();
			return(NULL);
		}
...
#if defined(_WIN32_WCE)
	if ( flags & SDL_FULLSCREEN ) video->flags |= SDL_FULLSCREEN;
#endif

#ifndef NO_CHANGEDISPLAYSETTINGS
	/* Fullscreen handling (без изменений) */
	if ( (flags & SDL_FULLSCREEN) == SDL_FULLSCREEN ) {
		DEVMODE settings;
		BOOL changed;
		SDL_memset(&settings, 0, sizeof(DEVMODE));
	...
	if ( ! (flags & SDL_OPENGL) ) {
		...
		/* Suss out the bitmap info header (без изменений) */
		binfo_size = sizeof(*binfo);
		if( is16bitmode ) {
			binfo_size += 3*sizeof(DWORD);
		} else if ( video->format->palette ) {
		...
		/* 1. Выделяем память под пиксели */
		video->pixels = SDL_malloc(video->h * video->pitch);
		if (!video->pixels) {
			ReleaseDC(SDL_Window, hdc);
			SDL_free(binfo);
			if (video != current) SDL_FreeSurface(video);
			SDL_OutOfMemory();
			return NULL;
		}
		SDL_memset(video->pixels, 0, video->h * video->pitch);
 /* ОДИН РАЗ переворачиваем буфер ЭМУЛЯТОРА в формат DIB */
    //ConvertToBottomUp(video->pixels, video->w, video->h, video->format->BitsPerPixel);
		/* 2. Создаём DDB из нашего DIB */
		screen_bmp = CreateDIBitmap(
			hdc,
			&binfo->bmiHeader,
			CBM_INIT,
			video->pixels,
		...
		/* 3. Сохраняем binfo для будущих вызовов */
		this->hidden->dib_bmi = binfo;

		ReleaseDC(SDL_Window, hdc);
		/* НЕ вызываем SDL_free(binfo) здесь! */
		...
		if ( screen_bmp == NULL ) {
			...
			SDL_SetError("Couldn't create DIBitmap");
			return(NULL);
		}

		this->UpdateRects = DIB_NormalUpdate;

		/* Set video surface flags (без изменений) */
		if ( screen_pal && (flags & (SDL_FULLSCREEN|SDL_HWPALETTE)) ) {
			grab_palette = TRUE;
		}
		if ( screen_pal ) {
	...
	/* Rest of function (без изменений) */
	DIB_ResizeWindow(this, width, height, prev_w, prev_h, flags);
	SDL_resizing = 0;
...
// {
	...
	// /* Синхронизируем DIB с DDB перед выводом */
	// if (this->hidden->dib_bmi) {
		// SetDIBits(hdc, screen_bmp, 0, this->screen->h,
				  // this->screen->pixels,
				  // this->hidden->dib_bmi,
				  // DIB_RGB_COLORS);
	// }
...
static void DIB_NormalUpdate(_THIS, int numrects, SDL_Rect *rects)
{
    ...
    pitch = this->screen->pitch;
    bpp = this->screen->format->BytesPerPixel;

    /* Выделяем временный буфер для bottom-up */
    bottom_buffer = (Uint8 *)SDL_malloc(this->screen->h * pitch);
    if (!bottom_buffer) {
        /* Если не можем выделить память — просто выходим */
        ReleaseDC(SDL_Window, hdc);
        return;
    }

    /* Копируем строки из top-down в bottom-up (меняем порядок) */
    for (i = 0; i < numrects; ++i) {
        SDL_Rect *r = &rects[i];
        
        for (y = 0; y < r->h; y++) {
            int src_y = r->y + y;                    /* top-down: y от верха к низу */
            int dst_y = this->screen->h - src_y - 1; /* bottom-up: y от низа к верху */
            
            memcpy(
                bottom_buffer + dst_y * pitch + r->x * bpp,
                (Uint8 *)this->screen->pixels + src_y * pitch + r->x * bpp,
                r->w * bpp
            );
        }
    }

    /* Обновляем DDB через SetDIBits (один раз для всех изменённых rects) */
    SetDIBits(hdc, screen_bmp, 0, this->screen->h,
              bottom_buffer, this->hidden->dib_bmi, DIB_RGB_COLORS);

    /* Выводим через BitBlt */
    mdc = CreateCompatibleDC(hdc);
    SelectObject(mdc, screen_bmp);
    for (i = 0; i < numrects; ++i) {
        BitBlt(hdc, rects[i].x, rects[i].y, rects[i].w, rects[i].h,
...
int DIB_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors)
{
	...
	/* Update the display palette (без изменений) */
	hdc = GetDC(SDL_Window);
	if ( screen_pal ) {
		PALETTEENTRY *entry;
	...
	/* Обновляем палитру в сохранённом BITMAPINFO */
	if (this->hidden->dib_bmi) {
		RGBQUAD *dst = this->hidden->dib_bmi->bmiColors;
		memcpy(dst + firstcolor, pal, ncolors * sizeof(RGBQUAD));
	...
	/* BitBlt остаётся как в оригинале */
	if ( moved_entries || !grab_palette ) {
		BitBlt(hdc, 0, 0, this->screen->w, this->screen->h,
			   mdc, 0, 0, SRCCOPY);
	}
