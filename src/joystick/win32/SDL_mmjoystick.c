...
void SDL_SYS_JoystickUpdate(SDL_Joystick *joystick)
{
	...
	/* ?Analog joyGetPos */
	result = joyGetPos(joystick->hwdata->id, &joyinfo);
	if ( result != JOYERR_NOERROR ) {
		SetMMerror("joyGetPos", result);
		return;
	}

	/* Analogs */
	pos[0] = joyinfo.wXpos;  /* ??? X ?????? ???? */
	pos[1] = joyinfo.wYpos;  /* ??? Y ?????? ???? */
	pos[2] = joyinfo.wZpos;  /* ??? Z ???? ?? ????????? ?????????? */
	

	if (joystick->naxes > 3) pos[3] = 0x3FFF; 
	if (joystick->naxes > 4) pos[4] = 0x3FFF;
	if (joystick->naxes > 5) pos[5] = 0x3FFF;

	/* joystick motion events */
	transaxis = joystick->hwdata->transaxis;
	for (i = 0; i < joystick->naxes; i++) {
		if (i < 3 || joystick->naxes > i) {
			value = (int)(((float)pos[i] + transaxis[i].offset) * transaxis[i].scale);
			change = (value - joystick->axes[i]);
			if ( (change < -JOY_AXIS_THRESHOLD) || (change > JOY_AXIS_THRESHOLD) ) {
				SDL_PrivateJoystickAxis(joystick, (Uint8)i, (Sint16)value);
	...
	/* joystick button events  joyinfo */
	for ( i = 0; i < joystick->nbuttons && i < 4; ++i ) {
		if ( joyinfo.wButtons & (1 << i) ) {
			if ( ! joystick->buttons[i] ) {
				SDL_PrivateJoystickButton(joystick, (Uint8)i, SDL_PRESSED);
			}
		} else {
	...
	if ( joystick->nhats > 0 ) {
		if ( joystick->hats[0] != SDL_HAT_CENTERED ) {
			SDL_PrivateJoystickHat(joystick, 0, SDL_HAT_CENTERED);
			joystick->hats[0] = SDL_HAT_CENTERED;
		}
	}
}
